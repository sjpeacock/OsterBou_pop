...
model5 <- function(){
	...
	for(i in 1:nT){
		...
		for(j in 2:3){
			p.mean[i,1] <- a[j]*exp(-E[j]/(8.62*10^-5)*(1/(T.obs[i]+273.15)-1/(15+273.15)))
		}
	}
...
model5 <- function(){
	...
	for(i in 1:nT){
		for(j in c(1,3)){
			p.mean[i,1] <- a[j]*exp(-E[j]/(8.62*10^-5)*(1/(T.obs[i]+273.15)-1/(15+273.15)))*(1+exp(Eh[j]/(8.62*10^-5)*(-1/(T.obs[i]+273.15)+1/(Th[j]+273.15))))^(z[j])
			}
		p.mean[i,2] <- a[2]
	}

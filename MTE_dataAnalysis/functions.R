...
sim.larv <- function(times = c(1:1200)/10, params){
		...
		#params <- list(N0, u0, u1, rho, sigma)
...
calcLik <- function(params.temp, dat, holdOut = NA){
	...
	for(i in 1:dat$nT){
		...
		# Add a small number to zero counts so that the -log likelihood is not -Inf
		if(length(which(N_pred[[i]] == 0)) > 0){
			N_pred[[i]][which(N_pred[[i]] == 0, arr.ind = TRUE)] <- 10^-4
		}
		}
...
# Calculate AIC
###############################################################################
calcAIC <- function(nLL, nParams, k = 2){
	return(2*nLL + k*nParams)
}